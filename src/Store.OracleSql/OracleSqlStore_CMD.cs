...
partial class OracleSqlStore
{
    protected override DbCommand BuildUpdateCommand(SqlUpdateCommand updateCommand, EntityModel model)
    {
        ...
        string cmdText = $"UPDATE {model.SqlStoreOptions!.GetSqlTableName(false, null)} t SET {BuildSet(updateCommand)} VALUES ";
    ...
    private string BuildSet(SqlUpdateCommand updateCommand)
    {
        string text = "(";
        foreach (var item in updateCommand.UpdateItems)
        {
    ...
    protected override DbCommand BuildDeleteCommand(SqlDeleteCommand cmd, EntityModel model)
    {
    ...
    public string GetParameterName(object value)
    {
        ...
        if (!Parameters.TryGetValue(value, out pname))
        {
            ...
            if (value is Guid guidValue)
                para.Value = guidValue.ToByteArray();
            else
                para.Value = value;
            if (value is string)
