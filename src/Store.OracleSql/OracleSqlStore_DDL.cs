...
partial class OracleSqlStore
{
    protected override IList<DbCommand> MakeCreateTable(EntityModel model, IModelContainer ctx)
    {
        ...
        //CONSTRAINT "FK_UserLogins_Users_UserId" FOREIGN KEY("UserId") REFERENCES "WXF"."Users"("Id")
        //)';
        //END;
        // 创建索引
        //CREATE INDEX "WXF"."IX_Users_UserName" ON "WXF"."Users"("UserName")
        var tableName = $"{model.SqlStoreOptions!.GetSqlTableName(false, ctx)}";
        var sb = StringBuilderCache.Acquire();
        //sb.Append("BEGIN");
        //sb.AppendLine();
        //sb.Append("EXECUTE IMMEDIATE 'CREATE TABLE");
        //sb.AppendLine();
        ...
        foreach (var mm in model.Members)
        {
            ...
            else if (mm.Type == EntityMemberType.EntityRef)
            {
                ...
                if (!rm.IsAggregationRef) //只有非聚合引合创建外键
                {
                    ...
                    sb.Append(BuildForeignKey(rm, ctx, tableName));
                    sb.Append(",");
                }
            }
        }
        ...
        if (model.SqlStoreOptions!.HasPrimaryKeys)
        {
            ...
            sb.AppendLine();
            sb.Append($"CONSTRAINT \"PK_{model.Id}\"");
        ...
        {
            new OracleCommand(StringBuilderCache.GetStringAndRelease(sb))
        };
    ...
    protected override IList<DbCommand> MakeAlterTable(EntityModel model, IModelContainer ctx)
    {
        ...
        if (model.SqlStoreOptions.IsTableNameChanged) //TODO:应判断存储选项的表名称是否改变
        {
            ...
            var renameTableCmd = new OracleCommand($"ALTER TABLE \"{oldTableName}\" RENAME TO \"{tableName}\"");
        ...
        if (deletedMembers.Length > 0)
        {
            ...
            if (needCommand)
            {
                ...
                commands.Add(new OracleCommand(cmdText));
            }
        ...
        if (addedMembers.Length > 0)
        {
            ...
            foreach (var m in addedMembers)
            {
                ...
                else if (m.Type == EntityMemberType.EntityRef)
                {
                    ...
                    if (!rm.IsAggregationRef) //只有非聚合引合创建外键
                    {
                        string fk = $"{BuildForeignKey(rm, ctx, tableName, true)};";
                        fks.Add(fk);
            ...
            foreach (var m in changedMembers)
            {
                if (m.Type == EntityMemberType.EntityField)
                {
                    ...
                    if (dfm.IsFieldTypeChanged)
                    {
                        ...
                        commands.Add(new OracleCommand(StringBuilderCache.GetStringAndRelease(sb)));
                    }

                    //再处理重命名列
                    if (m.IsNameChanged)
                    {
                        var renameColCmd = new OracleCommand(
    ...
    protected override DbCommand MakeDropTable(EntityModel model, IModelContainer ctx)
    {
        ...
        return new OracleCommand($"DROP TABLE IF EXISTS \"{tableName}\"");
    }
    ...
    private static void BuildFieldDefine(EntityFieldModel dfm, StringBuilder sb, bool forAlter)
    {
        ...
        switch (dfm.FieldType)
        {
            case EntityFieldType.String:
                sb.Append(dfm.Length == 0 ? "NVARCHAR2(2000) " : $"NVARCHAR2({dfm.Length}) ");
                break;
            ...
            default:
                throw new NotImplementedException(
                    $"OracleSqlStore.BuildFieldDefine with type: {dfm.FieldType}");
        }
    ...
    private static string BuildForeignKey(EntityRefModel rm, IModelContainer ctx, string tableName, bool forAlter = false)
    {
        ...
        if (forAlter)
            rsb.Append($"ALTER TABLE \"{tableName}\" ADD ");
        rsb.Append($"CONSTRAINT \"{fkName}\" FOREIGN KEY (");
        ...
        // Oracle 好像不支持ON UPDATE
        //if (rm.UpdateRule != EntityRefActionRule.NoAction)
        //    rsb.Append($"ON UPDATE {GetActionRuleString(rm.UpdateRule)} ");
        if (rm.DeleteRule != EntityRefActionRule.NoAction)
            rsb.Append($"ON DELETE {GetActionRuleString(rm.DeleteRule)}");
    ...
    private static void BuildIndexes(EntityModel model, List<DbCommand> commands, string tableName)
    {
        ...
        if (model.PersistentState != PersistentState.Detached)
        {
            ...
            {
                commands.Add(
                    new OracleCommand($"DROP INDEX IF EXISTS \"IX_{model.Id}_{index.IndexId}\""));
            }
        }
