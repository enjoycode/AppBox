...
public static class SqlStoreInitiator
{
    public static async Task TryInitStoreAsync()
    {
        ...
        cmd1.CommandText =
            $"Select meta From {esc}sys.Meta{esc} Where meta={SqlMetaStore.Meta_Application} And id='{Consts.SYS_APP_ID.ToString()}'";
    ...
    public static async Task TryInitOracleStoreAsync()
    {
        var db = SqlStore.Default;
        var esc = db.NameEscaper;
        //暂通过查询判断有无初始化过
        ...
        //    " execute immediate 'CREATE TABLE \"sys.Meta\" (" +
        //    "\"ID\" NVARCHAR2(128) NOT NULL," +
        //    "\"META\" NUMBER(3) NOT NULL," +
        //    "\"DATA\" BLOB NOT NULL," +
        //    "\"MODEL\" NUMBER(3) NOT NULL," +
        //    "CONSTRAINT \"PK_sys.Meta\" PRIMARY KEY (\"ID\", \"META\")" +
        //    ")';" +
        //    "END;";
        cmd2.CommandText = "CREATE TABLE \"sys.Meta\" (" +
            "\"ID\" NVARCHAR2(128) NOT NULL," +
            "\"META\" NUMBER(3) NOT NULL," +
            "\"DATA\" BLOB NOT NULL," +
            "\"MODEL\" NUMBER(3) NOT NULL," +
