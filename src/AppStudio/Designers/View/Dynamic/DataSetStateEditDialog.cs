...
{
    public DataSetStateEditDialog(DynamicState state)
    {
        _state = state;
        ...
        //初始化状态
        _state.Value ??= new DynamicDataSetState();
        _service = new RxProxy<string>(
            () => ((DynamicDataSetState)_state.Value).Service,
            v => ((DynamicDataSetState)_state.Value).Service = v);
    }
