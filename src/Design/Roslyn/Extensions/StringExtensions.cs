...
public static class StringExtensions
{
    ...
    {
        return completion.IsValidCompletionStartsWithIgnoreCase(partial) ||
               completion.IsSubsequenceMatch(partial);
    }
    ...
    {
        return new string(completion.Where(c => c >= 'A' && c <= 'Z').ToArray()).StartsWith(
            partial.ToUpper());
    }
    ...
    public static bool IsSubsequenceMatch(this string completion, string partial)
    {
        ...
        return new string(completion.ToUpper().Intersect(partial.ToUpper()).ToArray()) ==
               partial.ToUpper();
    }
