...
/// 新建节点后返回给前端的结果
/// </summary>
public sealed class NewNodeResult : IBinSerializable
{
    ...
    {
        ws.WriteByte((byte)ParentNodeType);
        ws.WriteString(ParentNodeId);
        ws.WriteString(RootNodeId);
        ws.WriteInt(InsertIndex);
        
        ws.WriteByte((byte)NewNode.Type);
        NewNode.WriteTo(ws);
    }
