...
export class ColorUtils {
    private static ScaleAlpha(a: PixUI.Color, factor: number): PixUI.Color {
        let alpha = clamp((Math.floor(Math.round(a.Alpha * factor)) & 0xFF), 0,
            255);
    ...
    public static Lerp(a: Nullable<PixUI.Color>, b: Nullable<PixUI.Color>, t: number): Nullable<PixUI.Color> {
        ...
        let red = clamp((Math.floor(ColorUtils.LerpInt(a.Red, b.Red, t)) & 0xFF), 0,
            255);
        let green = clamp((Math.floor(ColorUtils.LerpInt(a.Green, b.Green, t)) & 0xFF), 0,
            255);
        let blue = clamp((Math.floor(ColorUtils.LerpInt(a.Blue, b.Blue, t)) & 0xFF), 0,
            255);
        let alpha = clamp((Math.floor(ColorUtils.LerpInt(a.Alpha, b.Alpha, t)) & 0xFF), 0,
            255);
