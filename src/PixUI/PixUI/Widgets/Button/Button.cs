...
namespace PixUI
{
    public sealed class Button : Widget, IMouseRegion, IFocusable
    {
        ...
        private Rect GetHoverBounds()
        {
            ...
            {
                var pt2Win = _iconWidget.LocalToWindow(0, 0);
                return Rect.FromLTWH(pt2Win.X, pt2Win.Y, _iconWidget.W, _iconWidget.H);
            }
            else
            {
                var pt2Win = LocalToWindow(0, 0);
                return Rect.FromLTWH(pt2Win.X, pt2Win.Y, W, H);
            }
        }
