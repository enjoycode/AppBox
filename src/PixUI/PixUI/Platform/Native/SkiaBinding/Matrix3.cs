...
namespace PixUI
{
    public unsafe partial struct Matrix3
    {
        ...
        public Matrix3(float[] values)
        {
            ...
            if (values.Length != Indices.Count)
                throw new ArgumentException(
                    $"The matrix array must have a length of {Indices.Count}.", nameof(values));
        ...
        public static Matrix3 CreateIdentity() =>
            new Matrix3 { scaleX = 1, scaleY = 1, persp2 = 1 };
        ...
        public static Matrix3 Concat(Matrix3 first, Matrix3 second)
        {
            Matrix3 target;
        ...
        public readonly Matrix3 PreConcat(Matrix3 matrix)
        {
            var target = this;
        ...
        public readonly Matrix3 PostConcat(Matrix3 matrix)
        {
            var target = this;
        ...
        public static void Concat(ref Matrix3 target, Matrix3 first, Matrix3 second)
        {
            fixed (Matrix3* t = &target)
            {
                SkiaApi.sk_matrix_concat(t, &first, &second);
            }
        }

        public static void Concat(ref Matrix3 target, ref Matrix3 first, ref Matrix3 second)
        {
            fixed (Matrix3* t = &target)
            fixed (Matrix3* f = &first)
            fixed (Matrix3* s = &second)
            {
        ...
        public readonly Point MapPoint(Point point) => MapPoint(point.X, point.Y);
        ...
        public readonly void MapPoints(Point[] result, Point[] points)
        {
            if (result == null)
        ...
        public readonly Point[] MapPoints(Point[] points)
        {
            if (points == null)
                throw new ArgumentNullException(nameof(points));
