...
namespace CodeEditor
{
    public sealed class LineSegment : ISegment
    {
        ...
        private void BuildParagraphByTokens(ParagraphBuilder pb, TextEditor editor,
            int startIndex, int endIndex)
        {
            ...
            for (var i = 0; i < _lineTokens!.Count; i++)
            {
                ...
                tokenOffset = editor.Document.PositionToOffset(
                    new TextLocation(tokenStartColumn, LineNumber));
                var tokenText =
                    editor.Document.GetText(tokenOffset, tokenEndColumn - tokenStartColumn);
        ...
        private void BuildParagraphByFoldings(ParagraphBuilder pb, TextEditor editor)
        {
            ...
            {
                var starts =
                    editor.Document.FoldingManager.GetFoldedFoldingsWithStartAfterColumn(line,
                        column);
                if (starts.Count <= 0)
                {
                    if (line == LineNumber)
